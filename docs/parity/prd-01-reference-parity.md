# PRD 01 Agent 通知：reference product CLI 模块对照矩阵

## 对照边界与产品决策

- 只对照黑色 CLI 模块内的工作区 / 事项侧栏、页签、终端面板、通知入口与通知中心；reference product 白色产品导航、Logo、登录引导和 Matou 窗口外壳不参加验收。
- 需求完整性以 `01-agent-通知.md` 为清单；同范围可见样式、入口、反馈时机和状态迁移以当前可运行 reference product 及其现存代码为基线。
- 2026-08-25 产品确认采用方案 A：当前 reference product 行为优先，PRD 只补充 reference product 尚缺的能力。下表将 PRD 与 reference product 冲突处明确记录为已确认产品基线，不再复制 PRD 旧描述。

## reference product 交互对照矩阵

| 用户场景 | 当前 reference product 基线 | Matou 实际结果 | 运行 / 自动化证据 | 差异结论 |
|---|---|---|---|---|
...
| 五秒冷却 | reference product 按 `Stop`、`Notification`、`OSCNotification` 三类来源分别限流；同一 `Notification` 来源即使文案分类不同也共享冷却 | 同一 Session + 同来源分类五秒内只接收一次 | provider mapper + store tests | 一致 |
...
| 四层未读回溯 | 工作区下拉有红点、事项有红色数量徽标、页签含红点、面板有蓝环 | 同行为；溢出页签菜单也保留红点 | reference product `ClaudeCodeView.vue`；split UI / notification UI tests | 一致 |
| 切换工作区 | 当前 reference product 会把被选择工作区的全部未读标为已读 | 同行为；不采用 PRD“只聚焦具体面板才清理”的旧规则 | store / UI integration tests | 按方案 A 一致 |
| 打开通知中心 | 位于工作区头部下方的左侧浮层，宽 382px，距 CLI 底部 5px，背景 `#212121`、圆角 12px | 同位置、尺寸、颜色、圆角与进入动效 | `reference/notification-center.png`、`matou/notification-center.png` 及两份 JSON | 一致 |
...
| 空态 | 使用 reference product 空态图片与 `暂无通知`，只保留声音开关和关闭入口 | 复用同一 reference product 资源与文案 | `no-xiaoxi.png`；center tests | 一致 |
...
- 当前 reference product 通知中心：`docs/acceptance/evidence/prd-01/reference/notification-center.png`。
- 当前 reference product 几何 / 样式采样：`docs/acceptance/evidence/prd-01/reference/notification-center.json`。
...
reference product 运行采证使用当前源码和隔离应用数据；为只呈现用户指定的黑色 CLI 对照范围，采证时移除了覆盖在 CLI 上方的外层登录引导，未修改 reference product 源码或 CLI 内部样式。reference product 与 Matou 的中心均实测为 382px 宽、`rgb(33, 33, 33)` 背景、12px 圆角；页面错误列表均为空。
