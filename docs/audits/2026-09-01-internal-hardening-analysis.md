...
**边界**：修复应保持已确认 Mockup 的展开时长和轨迹；若需要改变动画节奏，需与 reference product 基线对照后决策。
...
**需产品决策**：拖入后的可见文本和引用风格需与 reference product 做 1:1 对照；底层路径安全引用属于必须项。
...
| 鼠标离开后卡片仍展开 | 完整套件与单独复跑均失败 | 交互回归；按 Mockup/reference product 基线修复 |
...
当前实现的右键菜单只提供“移除节点…”，旧 PRD/reference product E2E 等待“删除会话”。二者的用户语义不同：

- **删除会话**：删除当前终端/会话，通常由现有 Scene 布局决定后续焦点。
- **移除节点**：面向会话图，可能连同后代分支一起处理。

**需产品决策**：推荐同时保留两个动作并明确范围；若产品只保留“移除节点”，需要更新 PRD、reference product 对照矩阵和所有恢复合同。
...
| D-09 | 删除会话与移除节点 | 两个动作并存，明确影响范围 | 兼容 reference product 心智与 DAG 分支管理；菜单多一个动作 |
...
- 大粘贴分块、拖入路径引用
- 删除会话/移除节点按产品决策闭合 reference product 对照矩阵
