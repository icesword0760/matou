...
1. 当前可运行 reference product 在 `terminalContextMenu.mjs` 中临时返回空菜单；同文件保留完整 Fork / Detach 逻辑，`ClaudeCodeView.vue`、`forkSession.js` 与启动层也保留完整实现。Matou 激活这套已存在的 reference product 交互，而不是复制当前隐藏状态。
...
详细双基线矩阵：`docs/parity/prd-06-reference-parity.md`。

## 3. 16 项用户行为验收台账

| # | 用户场景 | 当前用户结果 | 权威证据 | 状态 |
|---:|---|---|---|---|
| 1 | 可恢复 Claude 面板内容区右键 | 在终端内容区直接打开菜单，同时显示 Fork 与独立窗口，顺序和文案与 reference product dormant UI 一致 | Electron 内容区右键回归 + 真实系统鼠标操作 | 通过 |
...
| 13 | Fork 失败 | 右侧面板显示供应方错误与 reference product 警示行 | Electron failure flow + `fork-failure.png` | 通过 |
...
- Runtime 覆盖：reference product 精确启动参数、一次消费、派生身份结算、恢复只 resume、自身身份丢失时失败、进程快速退出、Journal 单次警示、HUD 恢复可派生；另覆盖状态栏临时身份不提前开放 Fork、Fork / Resume 的状态栏确认解除启动超时、SessionEnd 在 PTY 退出后的短暂收尾窗口。
...
- 运行证据：
  - `docs/acceptance/evidence/prd-06/reference/runnable-menu-hidden.png`
  - `docs/acceptance/evidence/prd-06/reference/fork-source-baseline.json`
...
PRD 06 的需求台账、reference product 双基线与真实 Electron 行为已经闭合，等待产品验收。
