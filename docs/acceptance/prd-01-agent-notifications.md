...
- 后台面板完成、求助、等待或出错时，会出现蓝色面板光圈、红色上层标记和通知中心条目；声音开启且事件允许时播放与 reference product 相同的两音提示。
...
2026-08-25 产品确认采用方案 A：当前 reference product 行为优先，PRD 只补齐缺失能力。因此最终用户行为为：

1. 通知中心是工作区头部下方的 **左侧 382px 浮层**，不是右侧抽屉。
2. 点击浮层外部或按 Esc 会收起；当前 reference product 头部只有清空和关闭，没有“全部已读”。
...
详细双基线矩阵：`docs/parity/prd-01-reference-parity.md`。
...
| 2 | 聚焦面板产生通知 | 已读记录、无声音、无上层未读，保留 reference product 蓝环直到点击 | Electron 场景 2 + store tests | 通过（方案 A） |
...
| 8 | 批量清理 | 当前 reference product 提供“清空”，一次移除全部条目与层级未读 | center tests | 通过（方案 A，无“全部已读”入口） |
...
| 13 | 通知中心空态 | reference product 原图 + `暂无通知`，只保留声音开关 | center tests | 通过 |
| 14 | 切换工作区 | 当前 reference product 会清理被选工作区全部未读 | UI integration | 通过（方案 A） |
...
- 当前 reference product 与 Matou 的通知中心均完成真实 Electron 运行采样；两者中心实测为 382px 宽、`#212121` 背景、12px 圆角，列表层级、卡片、清空、关闭、底部声音开关一致。
...
- 运行截图与几何采样：
  - `docs/acceptance/evidence/prd-01/reference/notification-center.png`
  - `docs/acceptance/evidence/prd-01/reference/notification-center.json`
...
PRD 01 的实现与 reference product 双基线矩阵已闭合，等待产品验收。
