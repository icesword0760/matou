...
#### E2E-LIFE-003 [P1] 仅移除父节点并重连后代
- **前置**：祖父 G 下有父 P，P 下有子 C；相关自有 Worktree 数可由投影确定。
- **步骤/期望**：1) 从 P 卡片右键菜单选择“移除节点…”；2) 确认框默认“仅移除当前节点”，另可选择“移除当前节点及全部后代”，两项分别展示准确会话数和可判断的自有 Worktree 数；3) 确认默认范围后 P 消失，C 重连 G；4) 重启后 P 不复活，C→G 关系保持；5) 可见验收窗口均位于副屏 Color LCD。

#### E2E-LIFE-004 [P1] 移除整条分支
- **前置**：P 下有 2 个子、3 个孙与 1 个更深节点，相关工作树存在。
- **步骤/期望**：1) 在 P 打开“移除节点…”并选择“移除当前节点及全部后代”，确认页列出准确会话、自有 Worktree 与重要状态数量；2) 取消 → 所有节点保留；3) 再次操作并确认 → P 与全部后代/连线消失；4) 重启后整支不复活；5) 检查文件系统 → 每个工作树和未提交修改保留。
