...
export function TerminalSurface(props: TerminalSurfaceProps) {
  ...
  useEffect(() => {
    ...
    const model = foregroundTerminalModels.acquire(sessionId, () => {
      // Keep xterm's built-in DOM renderer. Its measured glyph layout preserves
      // synthetic italic CJK at every supported zoom level; the WebGL atlas
      // clips those wide fallback glyphs and makes intact text look corrupted.
    ...
    settleTerminalResize = () => {
      resizeSettleTimer = undefined
      // Historical output and resize frames must be applied at their recorded
      // grids. Fitting xterm (and the live PTY) in the middle of that stream
      ...
      if (checkpointAfterRecoveryLayout) {
        checkpointAfterRecoveryLayout = false
