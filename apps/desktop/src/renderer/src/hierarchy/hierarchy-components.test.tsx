...
describe('Workspace and Task navigation', () => {
  ...
  it('renders all Workspaces as flat groups and creates a Task in the selected group', async () => {
    ...
    expect(screen.getByRole('button', { name: '新增工作空间' })
      .querySelector('svg')?.dataset.icon).toBe('square-pen')
    expect(screen.getByRole('button', { name: '通知中心' })
      .querySelector('svg')?.dataset.icon).toBe('bell')
    expect(screen.getByRole('group', { name: 'icesword 工作空间' })).toBeTruthy()
    expect(screen.getByRole('group', { name: 'Frontend 工作空间' })).toBeTruthy()
    expect(screen.getByRole('button', { name: '在 icesword 中新增事项' })
      .querySelector('svg')?.dataset.icon).toBe('plus')
    const workspaceMenuButton = screen.getByRole('button', { name: '工作空间菜单：icesword' })
    expect(workspaceMenuButton.dataset.icon).toBe('ellipsis')
    expect(workspaceMenuButton.childElementCount).toBe(0)
    const taskMenuButton = screen.getByRole('button', { name: '事项菜单：默认' })
    expect(taskMenuButton.dataset.icon).toBe('ellipsis')
    expect(taskMenuButton.childElementCount).toBe(0)
  ...
  it('keeps settings at the bottom of the sidebar beside the board entry', async () => {
    ...
    expect(within(toolbar).getByRole('button', { name: '看板' })
      .querySelector('svg')?.dataset.icon).toBe('columns-3')
    expect(within(toolbar).getByRole('button', { name: '设置' })
      .querySelector('svg')?.dataset.icon).toBe('settings-2')
  ...
  it('uses separate selection levels and separate pin/action slots', () => {
    ...
    expect(group.querySelector('.workspace-group__status .pin-icon')?.getAttribute('data-icon')).toBe('pin')
