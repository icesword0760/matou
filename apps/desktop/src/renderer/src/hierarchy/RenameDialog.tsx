...
export function RenameDialog(props: {
  ...
  const m = messages.hierarchyShell
  ...
  const overlay = <div className={`dialog-overlay${props.scope === 'session' ? ' is-session-scoped' : ''}`} onPointerDown={(event) => {
    ...
    <header><h2>{props.title ?? m.renameDialog.title}</h2><button className="dialog-close" aria-label={messages.common.close} onClick={props.onCancel}>×</button></header>
    <div className="rename-dialog-body"><label>
      ...
      <span className="rename-input-wrap"><input autoFocus maxLength={20} aria-label={props.label}
        placeholder={props.placeholder ?? m.renameDialog.placeholder(props.label)}
    ...
    <footer><button onClick={props.onCancel}>{messages.common.cancel}</button>
      <button className="dialog-primary" onClick={confirm}
        disabled={Boolean(error) && Boolean(trimmed)}>{m.confirmOk}</button></footer>
  </div></div>
