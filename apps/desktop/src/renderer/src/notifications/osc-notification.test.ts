...
describe('reference product OSC terminal notifications', () => {
  ...
  it('ignores other OSC codes and uses reference product fallback content', () => {
    expect(toOscNotification(7, 'file:///tmp')).toBeNull()
    expect(toOscNotification(9, '')?.body).toBe('终端通知')
  })

  it('bounds raw terminal content to the reference product 180-character body', () => {
