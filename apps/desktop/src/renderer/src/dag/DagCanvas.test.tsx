...
describe('DagCanvas', () => {
  ...
  it('pans smoothly, zooms, restores 100 percent and selects a node', async () => {
    ...
    const displaced = canvas.getAttribute('data-pan')
    await waitFor(() => expect(canvas.getAttribute('data-pan')).not.toBe(displaced))
    const panned = canvas.getAttribute('data-pan')
    await userEvent.setup().click(screen.getByRole('button', { name: '恢复 100%' }))
    expect(canvas.getAttribute('data-scale')).toBe('1')
    expect(canvas.getAttribute('data-pan')).not.toBe(panned)
    expect(canvas.getAttribute('data-pan')).toBe('70,40')
