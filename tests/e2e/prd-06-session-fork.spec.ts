...
test.describe('PRD 06 session fork', () => {
  ...
  test('forks a resumable Claude conversation on its right, isolates every branch, and restores once', async () => {
    ...
    try {
      ...
      await expect(firstForkPane.locator('.pane-title')).toHaveText('第一子分支')
  ...
  test('keeps a failed fork visible and inert instead of opening a Shell or reusing the source', async () => {
    ...
    try {
      ...
      await expect(failedCard).toContainText('父会话已失效')
      await expect(failedCard).toContainText('请返回父会话继续，或移除此失败节点后新建空会话')
      await expect(failedCard).not.toContainText('provider session not found')
